...
export const regenerateModuleAction = internalAction({
	...
	handler: async (ctx, args) => {
		...
		const existingModules = await ctx.runQuery(
		...
		try {
			...
			const generatedData = await ctx.runAction(
				internal.ai.generateBrandModule,
				{
...
export const cascadeUpdates = internalAction({
	...
	handler: async (ctx, args) => {
		...
		for (const moduleType of dependentModules as BrandModuleType[]) {
			...
			const now = Date.now();
			const existingOfType = await ctx.runQuery(
				internal.brandModules.getModulesForGeneration,
				{
					companyId: args.companyId,
				}
			...
			const moduleId = await ctx.runMutation(
				internal.brandModules.createQueuedModuleInternal as any,
				{
					companyId: args.companyId,
					type: moduleType,
					now,
