...
- 🚧 **FLAGSHIP LAUNCH BLOCKER (owner decision):** `publishSite` is still **app-owner-gated** — no customer can publish their own site yet. Opening it up is entangled with the Free-preview/Pro-publish model + billing (owner-gated: pricing + checkout-collection verification). Do NOT unilaterally open public publishing unattended (outward-facing, abuse surface). Needs owner: publish-access model + billing verified.
