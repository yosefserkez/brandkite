...
- ✅ **#2 Publishable site** (flagship): real on-brand landing page at `/s/:slug` from the brand (logo/palette/fonts/website copy) + owner Publish control. **LIVE at brandkite.co/s/brandkite**. Now **SSR'd** (server loader): crawlable body + real title/description/OG/Twitter/canonical in head (was client-only shell); logo og:image on a 7-day URL; JSON-LD Organization. **Template quality pass** (dogfooding our own design bar): removed uppercase eyebrow pill, 01/02/03 number tiles + card chrome, and glow-on-dark — brand's own logo/color/type now carry the page. Verified in-browser.
- ✅ **Publish access is per-company-owner** (CORRECTION of an earlier wrong note): `publishSite`/`unpublishSite` authorize `company.ownerId === userId`, and the studio's Publish control shows for any authenticated user. So **any signed-in user can already publish their own brand's site** — the flagship is usable by real customers now. Open question (owner, not a blocker): whether to gate publishing behind Pro (free currently publishes unlimited) — that's a monetization refinement tied to billing verification, not an access bug.
- ✅ **Custom site URLs + preview**: owners can rename their site slug (`setSiteSlug`, validated + unique) and preview the site pre-publish (`getSitePreview`, owner-gated) at `/s/preview/:companyId`.
